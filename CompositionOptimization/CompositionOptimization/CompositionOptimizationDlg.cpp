...
void CCompositionOptimizationDlg::InitMenu()
{
	...
	for (int i = 0; i < m_nNumber; i++)
	{
		CMenu subMenu1;
		subMenu1.CreateMenu();
		vector<CString> subCString;
		LoadStrFromIni(subCString, vtCString.at(i));
		...
		for (int j = 0; j < subNumber; j++)
		{
			...
			CString enName = subCString.at(j).Mid(a + 1, b - a - 1); //2.取出逗号后的英文名
			CString choiceValue = subCString.at(j).Right(subCString.at(j).GetLength() - b - 1);//3.取出性质的大小值信息
...
void CCompositionOptimizationDlg::OnAddComposition(UINT nID)
{
	...
	AutoSizeListColumn(m_CalcRatioList, 0);//调整计算系数列表的列宽
	int rColumn = m_ResultList.GetHeaderCtrl()->GetItemCount();//调整结果列表的宽度
...
void CCompositionOptimizationDlg::OnAddNature(UINT nID)
{
	...
	AutoSizeListColumn(m_NatureList, 1);//调整性质列表的列宽
	AutoSizeListColumn(m_CalcRatioList, 0);//调整计算系数列表的列宽
	int rColumn = m_ResultList.GetHeaderCtrl()->GetItemCount();//调整结果列表的宽度
	for (int i = 0; i < rColumn; i++)
	{
